...
class Sensing {
  ...
  void start() async {
    ...
    study.tasks.add(movisenTask);

    // print the study to the console
    console.log(study.toString());

    // create a new executor
    controller = new StudyController(study);
    ...
    if (_appTask == null)
      _appTask = Task("Application Task")
        ..addMeasure(PeriodicMeasure(MeasureType(NameSpace.CARP, DataType.APPS),
            name: "Installed apps", frequency: 30 * 1000));
    ...
    if (_appUsageTask == null) {
      _appUsageTask = Task("AppUsage Task")
        ..addMeasure(PeriodicMeasure(
            MeasureType(NameSpace.CARP, DataType.APP_USAGE),
  ...
  Task get connectivityTask {
    if (_connectivityTask == null) {
      _connectivityTask = Task("Connectivity Task")
        ..addMeasure(Measure(MeasureType(NameSpace.CARP, DataType.CONNECTIVITY))
          ..name = "Connectivity")
        ..addMeasure(
    ...
    if (_hardwareTask == null) {
      _hardwareTask = Task("Hardware Task")
        ...
        ..addMeasure(Measure(MeasureType(NameSpace.CARP, DataType.BATTERY))
          ..name = "Battery")
        ..addMeasure(Measure(MeasureType(NameSpace.CARP, DataType.SCREEN))
  ...
  Task get sensorTask {
    if (_sensorTask == null) {
      _sensorTask = Task("Sensor Task")
        ..addMeasure(
            PeriodicMeasure(MeasureType(NameSpace.CARP, DataType.ACCELEROMETER),
                name: "Accelerometer",
                frequency: 8 * 1000, // How often to start a measure
                duration: 20 // Window size
                ))
        ..addMeasure(
            PeriodicMeasure(MeasureType(NameSpace.CARP, DataType.GYROSCOPE),
                name: "Gyroscope",
                frequency: 9 * 1000, // How often to start a measure
                duration: 100 // Window size
                ))
        ..addMeasure(
            PeriodicMeasure(MeasureType(NameSpace.CARP, DataType.LIGHT),
                name: "Light",
